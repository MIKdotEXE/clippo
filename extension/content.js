...
function createWidget(video) {
  ...
  if (!document.getElementById("vm-style")) {
    ...
    style.textContent = `
      ...
      #clippo-widget .pw-field {
        position: relative;
      }
      #clippo-widget .pw-field input {
        width: 100%;
        padding-right: 36px;
      }
      #clippo-widget .pw-toggle {
        position: absolute;
        right: 6px;
        top: 50%;
      ...
      #clippo-widget .pw-toggle:hover { opacity: 1; }
      #clippo-widget .pw-toggle .eye-hide { display: none; }
      #clippo-widget .pw-toggle.showing .eye-show { display: none; }
      #clippo-widget .pw-toggle.showing .eye-hide { display: block; }
...
function renderAuthForm() {
  widget.innerHTML = `
    ...
    <button class="vm-btn-secondary" id="vm-google-btn" style="width:100%;display:flex;align-items:center;justify-content:center;gap:6px;margin-bottom:10px;padding:8px;">
...
function bindAuthForm() {
  ...
  // Google OAuth
  $("vm-google-btn").onclick = () => {
    try {
      chrome.tabs.create({ url: 'https://clippo.app/auth/?google=1' });
    } catch (e) {
      window.open('https://clippo.app/auth/?google=1', '_blank');
    }
  };
  ...
  widget.querySelectorAll('.pw-toggle').forEach(btn => {
    btn.addEventListener('click', () => {
